...
public final class EitherCodec<F, S> implements Codec<Either<F, S>> {
	...
	public <T> DataResult<Pair<Either<F, S>, T>> decode(final DynamicOps<T> ops, final T input) {
		...
		return DataResult.error(() -> "Input does not match either left or right: %s, first error: %s, second error: %s".formatted(
				input,
				firstRead.error().orElseThrow(),
				secondRead.error().orElseThrow()));
	}
